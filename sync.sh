...
# Unfortunate special casing until I can figure out how to make s3cmd sync non-recursively.
"${s3cmd}" sync --acl-public --delete-after --exclude '.git/' "${localparentdir}/*.html" "${remotedest}" >> "${synclog}" 2>> "${errorlog}"
"${s3cmd}" sync --acl-public --delete-after --exclude '.git/' "${localparentdir}/*.css" "${remotedest}" >> "${synclog}" 2>> "${errorlog}"
date >> "${synclog}"
date >> "${errorlog}"
echo "--END--" >> "${synclog}"
echo "--END--" >> "${errorlog}"
