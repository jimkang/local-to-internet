...
"${s3cmd}" put --acl-public --delete-after --exclude '.git/' "${localparentdir}/*.html" "${remotedest}" >> "${synclog}" 2>> "${errorlog}"
"${s3cmd}" put --acl-public --delete-after --exclude '.git/' "${localparentdir}/*.css" "${remotedest}" >> "${synclog}" 2>> "${errorlog}"
date >> "${synclog}"
date >> "${errorlog}"
echo "--END--" >> "${synclog}"
echo "--END--" >> "${errorlog}"
